...
searchBtn.addEventListener("click", async () => {
    ...
    if (filteredV.length === 0) {
        ...
        filteredV.forEach(vehicle => {
            modalBody.innerHTML += `
                <div class="my-1 container d-flex f-row justify-content-between align-items-center gap-2">
                    ...
                    <a id="${vehicle.id}" onclick="localStorage.setItem('vehicleID', this.id);" class="btn btn-primary" href="./pages/vehicle.html">VER</a>
                </div>
                <hr>
            `
        });
